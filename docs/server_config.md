...
The BLE server uses the default advertising string of "**Tmx4Pico2W**". 
It is defined near the top of the BLE server sketch.
...
![](images/ble_server.png){ width="1000" }
...
### Setting The Advertising String In The Telemetrix4RpiPico2w-BLE Python Class

Set the ble_device_name parameter to match the value set in the BLE server sketch.
...
### Setting The Advertising String In The TelemetrixRpiPico2wBleAio Python Class
```aiignore
...
Before compiling a BLE server, you must configure the Arduino IDE to include the BLE stack. To do so, in the Arduino IDE, select Tools > IP > Bluetooth Stack, then IPV4 + Bluetooth.

<br>
![](images/ble_server_2.png){ width="1000" }
...
![](images/config_wifi_server.png){ width="1000" }
...
```
The first two lines are BLE service UUIDS.
The arduino-pico library generates them. The first is the battery service, and
the second is the Nordic UART service.
...
```
You must set the SSID and Password to match that of your router. A reminder is displayed for you to do that.

It may take several seconds for the Pico2w to connect to your router. A connection 
progress string is displayed. When you first power the Pico2w, the board LED lights and stays on until a successful connection to your router is established. 
The IP address assigned to the Pico2w by the router is displayed, along with the IP port number.

!!! warning

    Make sure to close the Arduino serial monitor after viewing the startup 
    information. If you are using the serial server, leaving it open will prevent the client from connecting.
