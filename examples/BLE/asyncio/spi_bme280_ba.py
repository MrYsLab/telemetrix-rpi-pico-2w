...
async def read_bme_temp(my_board):
    ...
    await board.set_pin_mode_spi(chip_select=6)

    # reset the bme280
    # the data to write must be presented in the form of a list
    await board.spi_write_blocking([bme_reset_register])

    # get the raw temperature
    await board.spi_read_blocking(bme_temp_register, 3, spi_port=0, call_back=device_callback)
